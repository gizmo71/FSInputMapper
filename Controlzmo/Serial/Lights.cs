...
namespace Controlzmo.Serial
{
    ...
    public class BeaconLight : ISettable<bool?>
    {
        ...
        public void SetInSim(ExtendedSimConnect simConnect, bool? value)
        {
            ...
            sender.Execute(simConnect, $"(A:LIGHT BEACON ON, Bool) {desiredValue} = if {{ (>K:TOGGLE_BEACON_LIGHTS) }}");
        }
    }
    ...
    public class WingIceLight : ISettable<bool?>
    {
        ...
        {
            var desiredValue = value == true ? 1 : 0;
            sender.Execute(simConnect, $"(A:LIGHT WING ON, Bool) {desiredValue} = if {{ (>K:TOGGLE_WING_LIGHTS) }}");
        }
    }
}
