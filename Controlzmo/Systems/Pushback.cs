...
namespace Controlzmo.Systems.Pushback
{
    ...
    public struct PushbackWaitData
    {
        ...
        // The EFB code suggests that after calling the tug, this variable is no longer useful, which matches observations, except that it seems useless before too!
    ...
    public class PushbackListener : DataListener<PushbackData>, IRequestDataOnOpen
    {
        ...
        {
            System.Console.Error.WriteLine($"Pushback: state={data.pushbackState}, attached={data.pushbackAttached}, "
                + $"angle={data.pushbackAngle}, heading={data.trueHeading}, velocity={data.velocityTotal}");
