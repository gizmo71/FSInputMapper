...
namespace Controlzmo.Systems.Transponder
{
    ...
    public class TransponderCodeListener : DataListener<TransponderCodeData>, IRequestDataOnOpen
    {
        ...
        {
            string asString = $"{data.binaryCodedOctal:X4}";
Console.Error.WriteLine($"Transponder code {asString}");
            hub.Clients.All.SetFromSim(TransponderCode.id, asString);
        }
    }
    ...
    public partial class AltRptg : DataListener<AltRptgData>, IRequestDataOnOpen, ISettable<bool?>
    {
        ...
        {
            if (simConnect.IsFenix) data.isAltRptgOn = data.isAltRptgOnFenix;
            else if (simConnect.IsIni321) data.isAltRptgOn = 1 - data.isAltRptgOnIni;
            else if (simConnect.IsIniBuilds) data.isAltRptgOn = data.isAltRptgOnIni;
            hub.Clients.All.SetFromSim(GetId(), data.isAltRptgOn == 1);
Console.Error.WriteLine($"Is Alt Reporting on? {data.isAltRptgOn == 1}");
        }

        public void SetInSim(ExtendedSimConnect simConnect, bool? isOn)
        {
            var value = isOn == true ? 1 : 0;
            if (simConnect.IsIni321) value = 1 - value;
...
/*TODO: doesn't even seem to do anything in the ATR :-(
 * Appears to default to 1 regardless of whether the transponder's actually on or not.
 * Screen shows "TCAS STBY" when XPNDRs are both off (or Alt Reporting is off) and "TA ONLY" on ground when they're on.
 * But we don't see any changes here. :-( */
    ...
    public class TcasMode : DataListener<TcasModeData>, IRequestDataOnOpen, ISettable<string?>
    {
        ...
        public override void Process(ExtendedSimConnect simConnect, TcasModeData data)
        {
            ...
            else if (simConnect.IsIniBuilds) data.position = data.positionIni;
            else if (simConnect.IsA380X) data.position = data.a380xMode;
            hub.Clients.All.SetFromSim(GetId(), data.position);
Console.Error.WriteLine($"TCAS mode position {data.position} or standard {data.standard}");
        }
        ...
        public void SetInSim(ExtendedSimConnect simConnect, string? posString)
        {
            ...
            simConnect.SendDataOnSimObject(new TcasModeData() {
                position = code, a380xMode = code, positionFenix = code, positionIni = code
            });
        }
    }
...
Console.Error.WriteLine($"TCAS traffic position {data.position}");
            hub.Clients.All.SetFromSim(GetId(), data.position);
        }
    ...
    public struct TransponderModeData
    {
        ...
        public Int32 xpndrMode; // 0 stby, 1 auto, 2 on
...
//TODO: sadly, not settable, but in the ATR we could play silly games pressing the appropriate buttons...
        [SimVar("TRANSPONDER STATE:1", "number", SIMCONNECT_DATATYPE.INT32, 0.5f)]
        public Int32 standard; // 0 off, 1 stby, 2 test, 3 on, 4 alt, 5 ground
    };
...
Console.Error.WriteLine($"Transponder mode {data.xpndrMode} or standard {data.standard}");
        ...
        {
            var code = Int16.Parse(posString!);
            if (simConnect.IsIni321 && code == 2) code = 1;
